...
- ✅ 素材库统一管理
- ✅ 本地存储支持
- ✅ SQLite 持久任务队列，刷新页面后仍可恢复后台任务状态
- ✅ Task Center 展示并行任务、失败重试、取消和父子任务进度
...
长耗时创作动作会写入 `creation_tasks`，包括 Agent、图片、视频、TTS、宫格、合成和拼接。任务状态可通过前端 Task Center 或 API 查看：

```bash
curl "http://localhost:5679/api/v1/tasks?drama_id=<DRAMA_ID>&episode_id=<EPISODE_ID>"
curl "http://localhost:5679/api/v1/task-audit/stuck"
```

维护文档：

- 后端实现说明：`backend/src/services/tasks/README.md`
- 运维与 QA：`docs/task-system-runbook.md`
