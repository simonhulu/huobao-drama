...
| key | 中文名 | 适用内容 | 核心规则 |
|-----|--------|----------|----------|
| `cold_eastern_epic` | 冷调东方史诗 | 战争、权谋、王朝更替、宫城攻防 | 3D建立空间、建筑、雾气和材质；2D统一墨线、平涂色块、木版画颗粒 |
| `cold_ink_spatial` | 冷墨史诗·三维空间二维绘 | 王朝覆灭、政变现场、战争关键帧、高冲击开场 | 3D负责空间、光影、景深和材质；2D负责角色、轮廓、剪影、阴影块面和颗粒 |

两者都使用冷青、墨绿、深靛蓝的环境色，并以暗红旗帜和橙金火光作为少量叙事强调色。`cold_eastern_epic` 更适合宏大历史场面；`cold_ink_spatial` 更强调二维角色嵌入三维场景，适合政变、战争开场和不可逆的权力冲突。
...
| key | 中文名 | 构图与用途 |
|-----|--------|------------|
| `dragon_rider_cover` | 赤龙骑士·史诗封面 | 449:600 竖幅，低机位龙骑士封面；强调金色磨损铠甲、红棕鳞片、逆光轮廓与骑士/巨龙的尺度关系 |
| `volcanic_forge_blacksmith` | 熔岩锻造·铁匠史诗 | 4:5 竖幅，低机位锻造特写；强调熔钢、铁砧、熔岩瀑布、烟雾、皮革和铸铁材质 |

这两个 preset 已加入后端 `STYLE_DESCRIPTIONS`，并在前端新建剧集的视觉风格选择器中可选。它们保存的是完整的英文视觉指令，而不是简单的色板标签，因此会同时约束主体、镜头、光线、材质、色彩和生成意图。

按 gpt-image-2 low quality 1024×1024 约 $0.006/张估算，生成 20 张样例约 $0.12，成本可控。

## 八、Dharma 情绪风格 preset

Dharma 管线使用独立的 `/api/v1/dharma/image-styles` 目录管理运镜、treatment、情绪映射和
预览图；三个 ID 同时注册在通用 `visual-style.ts`，因此脚本或通用图片工具不会把它们降级为
无语义的 `"<id> style"`：

| key | 中文名 | 情绪职责 |
|-----|--------|----------|
| `dharma-emotional-ink-v1` | 水墨意境 | `stillness / acceptance / release` |
| `dharma-surreal-dream-v1` | 超现实梦境 | `curiosity / tension` |
| `dharma-minimal-light-v1` | 极简光影 | `insight` 与金句停顿 |

新生产的完整六节点弧线固定按情绪映射使用这三种风格，不引入第四种；旧 Dharma 风格只保留读取兼容，
不出现在 production 选择器中。预览图为项目内本地 16:9 无字资产，不能替代正式素材审片。
