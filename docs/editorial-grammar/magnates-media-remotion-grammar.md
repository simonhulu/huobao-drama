...
The strongest visual calibration is Yahoo. The frozen Yahoo review set contains 25 target events across 10 review units, and the latest high-resolution run passes 25/25 targets and 10/10 reviews:

`tmp/editorial-analysis/vlm-calibration-yahoo-highres-merged-v7/calibration-report.json`
...
Corpus verification: `tmp/editorial-analysis/review-assets-v2-verification.json`. Semantic verification: `tmp/editorial-analysis/semantic-outlines-verification.json`.
...
The machine-readable contract is `docs/editorial-grammar/magnates-media-recipe.schema.json`. The executable TypeScript types are in `remotion/src/editorial/types.ts`.
...
The production props builder accepts an explicit recipe file and never guesses
Magnates-style animation from a legacy project:

```bash
node scripts/videoeditor/build_remotion_project_props.mjs \
  --project-id <remotion-project-id> \
  --recipe path/to/magnates-recipe.json \
  --output data/temp/magnates-props.json

cd remotion
npx remotion render src/index.tsx MagnatesEditorial out/magnates.mp4 \
  --props=../data/temp/magnates-props.json \
...
`EpisodeShowcase` and `TemporalGridEpisode` projects keep their current route.

## Implemented demo

Composition: `MagnatesEditorial`, 1280x720, 30fps, 525 frames.

It exercises five connected setups: hook contrast and counter, line-art/globe layer assembly, blur bridge into comparison UI, black matte chapter reveal, and chromatic reversal with type-on plus persistent metric. The demo uses existing local assets under `remotion/public`; no external editor or runtime is required.
